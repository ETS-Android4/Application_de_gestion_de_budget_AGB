...
public class HealthTransaction extends AppCompatActivity {
    ...
    public static String Healthaff="0";

    private ImageButton retourHe;

    private int sommeHealth = 0;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_health_transaction);

        retourHe = findViewById(R.id.imageButtonHe);
        retourHe.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                //animation button
                AlphaAnimation alpha = new AlphaAnimation(0f, 1f);
                alpha.setDuration(200);
                retourHe.startAnimation(alpha);
                //end animation button
                Intent ret = new Intent(HealthTransaction.this, MyExpenses.class);
                startActivity(ret);
            }
        });
        ...
        ADDHealth = findViewById(R.id.buttonAddHealth);
        ADDHealth.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                ADDHealth.startAnimation(alphaADDH);
                //end animation button
                ...
                if (set == null) {
                    ...
                    sommeHealth = sommeHealth + parseInt(sommeT);
                    Recuperer HealthExpenses = new Recuperer(sommeHealth);
                    Healthaff = String.valueOf(HealthExpenses.Afficher());
                    //TransactionList.add(aff);
                    Ref.child(userID).child("Health").setValue(sommeHealth);
                } else {
                    TransactionList = new ArrayList(set);
                }


                final ArrayAdapter arrayAdapter = new ArrayAdapter(HealthTransaction.this, android.R.layout.simple_list_item_1, TransactionList);
                listView.setAdapter(arrayAdapter);

                //sommeHealth = sommeHealth + parseInt(sommeT);
                //Recuperer HealthExpenses = new Recuperer(sommeHealth);
                //HealthExpenses.Afficher();
            }
        });
